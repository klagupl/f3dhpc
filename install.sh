...
yum install epel-release mc nano net-tools nfs-utils dhcp tftp httpd syslinux pdsh openssh-server firewalld tftp-server git xinetd tftp-server syslinux-tftpboot wget vsftpd opensm pdsh infiniband-diags -y
...
echo HOSTNAME=head.local > /etc/sysconfig/network
